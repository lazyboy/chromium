...
var initBotsToClick = function() {
  ...
  // Raw list copied from a successful tryserver run:
  ...
  // Manual list from |ALL_RAW_CQ_BOTS|.
  var ALL_CQ_BOTS = {
    'gpu': [
      'win_gpu', 'mac_gpu', 'linux_gpu'
    ],
    'linux': [
      'android_dbg_tests_recipe',
      'chromium_presubmit',
  ...
  botDivIDs = [];
  for (var bot_os in ALL_CQ_BOTS) {
    for (var j = 0; j < ALL_CQ_BOTS[bot_os].length; ++j) {
      //LOG('try find: ' + ALL_CQ_BOTS[bot_os][j] + ', under: ' + bot_os);
      var botDivToClick = 'cb_tryserver.chromium.' + bot_os + '_' + ALL_CQ_BOTS[bot_os][j];
      botDivIDs.push(botDivToClick);
    }
  }
  BOTS_TO_CLICK = botDivIDs;
  LOG('Init complete, total bot checkboxes to click', BOTS_TO_CLICK.length);
};
...
var init = function() {
  ...
  var trybotButtonDiv = document.getElementsByClassName('trybot-button-div')[0];
  if (!trybotButtonDiv) {
    LOG('*** ERROR: Cannot find div with class name trybot-button-div');
    return;
  }

  trybotButtonDiv.appendChild(button);

/*
  var trybotPopupDiv = document.getElementById('trybot-popup-checkboxes');
  if (!trybotPopupDiv) { LOG('trybot-popup-checkboxes div not found'); return; }
  ...
  for (var i = 0; div = trybotPopupBuildersDivs[i]; ++i) {
    ...
    for (j = 0; j < 10; ++j, candidateDiv = candidateDiv.nextSibling) {
      if (candidateDiv.innerText == 'client.skia') {
        LOG('found client.skia div');
        break;
      }
    }
    if (j < 10) break;
...
/**
linux_chromium_rel_swarming   linux_chromium_chromeos_rel_swarming   win_gpu_triggered_tests   win8_chromium_rel   win_gpu   win_chromium_rel_swarming   android_dbg_tests_recipe   win_chromium_x64_rel_swarming   mac_chromium_rel_swarming   android_aosp   win_chromium_compile_dbg   mac_gpu_triggered_tests   mac_gpu_retina_triggered_tests   linux_gpu_triggered_tests   mac_gpu   linux_chromium_gn_rel   linux_gpu   ios_dbg_simulator   android_clang_dbg   mac_chromium_compile_dbg   android_chromium_gn_compile_rel   ios_rel_device   ios_rel_device_ninja   win_chromium_rel   mac_chromium_rel   linux_chromium_chromeos_rel   linux_chromium_rel   chromium_presubmit 
**/
